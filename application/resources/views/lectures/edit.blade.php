...
@section('content')
    ...
    <form method="post" action="{{ route('courses.lectures.update', [ $course->id,$lecture->id]) }}">
        ...
        <div class="form-group">
            ...
            <input id="name" name="name" autocomplete="off" placeholder="Name" class="form-control" value="{{ $lecture->name }}" aria-describedby="nameHelpBlock" required="required" type="text">
            <span id="nameHelpBlock" class="form-text text-muted">Lecture name</span>
        </div>

        <div class="form-group">
            <label for="notes">Notes</label>
            <textarea id="notes" name="notes" cols="40" rows="2" class="form-control"  aria-describedby="notesHelpBlock">{{ $lecture->notes }}</textarea>
            <span id="notesHelpBlock" class="form-text text-muted">notes</span>
        </div>

        <div class="form-group">
            <label for="start_datetime">Start datetime</label>
            <input id="start_datetime" name="start_datetime" autocomplete="off" class="form-control datetime" aria-describedby="nameHelpBlock" value="{{$lecture->start_datetime->format('Y/m/d H:i') }}" required="required" type="text">
            <span id="nameHelpBlock" class="form-text text-muted">start date</span>
        </div>

        <div class="form-group">
            <label for="end_datetime">End datetime</label>
            <input id="end_datetime" name="end_datetime" autocomplete="off" class="form-control datetime" aria-describedby="nameHelpBlock" value="{{ $lecture->end_datetime->format('Y/m/d H:i') }}" required="required" type="text">
